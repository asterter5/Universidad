    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not self.walls[nextx][nexty]:
                nextState = (nextx, nexty)
                cost = self.costFn(nextState)
                if nextState in cLeft:
                    cLeft.remove(nextState)
                successors.append( ( (nextState, cLeft), action, cost) )
...
def cornersHeuristic(state, problem):
    ...
    while len(remainingCorners) > 0:
        distance, corner = min([(util.manhattanDistance(currentNode, c), c) for c in remainingCorners])
        sum += distance
        currentNode = corner
    ...
    for i in range(len(foodList)):
        for food in foodList:
            ...
            if closestCost > cost:
                closestCost = cost
                closestFood = food
        newCost = util.manhattanDistance(position, closestFood)
