...
/**
 *
 *  Function used to read a tree from a file. Do not call directly.
 *
 */

static void loadTreeRecursive(RBTree * tree, FILE * file) {
    int listlen = 0;
    int end = 1;
    ...
    while(end) {
        ...
        fread(key, sizeof(char), 3, file);

        if (!feof(file)) {
            nodeData = (RBData *) malloc(sizeof(RBData));
            nodeData->key = (char * ) malloc(sizeof(char) * 4);
            nodeData->aeropuertosDestino = (List *) malloc(sizeof(List));
            initList(nodeData->aeropuertosDestino);

            memcpy(nodeData->key, key, 4);
            fread(&listlen, sizeof(int), 1, file);
            printf("Llegint llista de longitud: %d\n", listlen);
            for (int i=0; i<listlen; i++){
                current = (ListData *) malloc(sizeof(ListData));
                current->key = (char *) malloc(sizeof(char) * 4);
                current->key_sec = (char *) malloc(sizeof(char) * 4);
                current->numeroRetrasos = (int*) malloc(sizeof(int)*7);
                current->totalRetrasos = (int*) malloc(sizeof(int)*7);

                fread(current->key, sizeof(char), 3, file);
                memcpy(current->key_sec, nodeData->key, 4);
                for (int dia=0; dia < DIES_SETMANA; dia++) {
                    fread(&current->totalRetrasos[dia], sizeof(int), 1, file);
                    fread(&current->numeroRetrasos[dia], sizeof(int), 1, file);
                }

                insertList(nodeData->aeropuertosDestino, current);
                //dumpList(nodeData->aeropuertosDestino);
            }
...
/**
 *
 *  Reads a tree. All the nodes and all the data saved in the file is saved to the tree.
 *
 */

void loadTree(RBTree *tree, FILE * file) {
    loadTreeRecursive(tree, file);
}

/**
 *
 *  Function used to save a tree to file. Do not call directly.
 *
 */

static void saveTreeRecursive(Node *x, FILE * file) {
    int listlen = 0;
    ...
    if (x->right != NIL) {
        saveTreeRecursive(x->right, file);
    }

    if (x->left != NIL) {
        saveTreeRecursive(x->left, file);
    }

    list = x->data->aeropuertosDestino;
    listlen = listLength(list);

    printf("Guardant llista de longitud: %d\n", listlen);
    //dumpList(list);

    fwrite(x->data->key, sizeof(char), 3, file);
    fwrite(&listlen, sizeof(int), 1, file);

    current = list->first;
    while (current != NULL) {
        fwrite(current->data->key, sizeof(char), 3, file);
        for (int i = 0; i < DIES_SETMANA; i++) {
            fwrite(&current->data->totalRetrasos[i], sizeof(int), 1, file);
            fwrite(&current->data->numeroRetrasos[i], sizeof(int), 1, file);
        }
...
/**
 *
 *  Saves a tree. All the nodes and all the data pointed to by
 *  the tree is saved to file.
 *
 */

void saveTree(RBTree *tree, FILE * file) {
    if (tree->root != NIL)
        saveTreeRecursive(tree->root, file);
}
