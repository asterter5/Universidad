...
void initialize(void){
	TRISA = 0x0000;//configura el port A (on est? connectada la bateria de LEDS com a sortida
...
int main(void){
	...
	while(1){
		if((PORTD & 0x0040)==0x0000)//mira si el switch S3 esta apretat. si es correcte entra en el bucle
			PORTA ^=0xff;
		}
	return 0;
}
